...
async function getGoogleToken() {
  // 1. Intentar con OAuth2 (Refresh Token) si existe (Evita error de cuota)
  if (process.env.GOOGLE_REFRESH_TOKEN && process.env.GOOGLE_CLIENT_ID) {
    ...
    } catch (e) { console.error("OAuth2 failed, falling back to Service Account"); }
  }
...
async function uploadToDrive(filename: string, content: string) {
  ...
  if (!token) return "❌ Error: Sin credenciales configuradas";
  try {
    ...
    if (e.message.includes("quota")) return "⚠️ Cuota: Google bloquea subida por ser cuenta personal. Use Refresh Token o descarga local.";
  ...
  try {
    ...
    if (action === "maintenance") {
      ...
      await transporter.sendMail({
        ...
        html: `
          <div style="font-family: sans-serif; max-width: 600px; margin: auto; border: 1px solid #e2e8f0; border-radius: 20px; overflow: hidden; background: #ffffff; color: #334155;">
            <div style="background: #0f172a; color: white; padding: 40px 20px; text-align: center;">
              ...
              <p style="color: #94a3b8; margin-top: 10px;">Estado General: <b>ÓPTIMO</b></p>
            </div>
            
            <div style="padding: 30px;">
              <h3 style="color: #0f172a; border-bottom: 2px solid #f1f5f9; padding-bottom: 8px;">1. Acciones Realizadas</h3>
              <table style="width: 100%; font-size: 13px;">
                <tr><td style="padding: 4px 0;">Registros de log eliminados:</td><td style="text-align: right; font-weight: bold; color: #3b82f6;">${acciones.logs_eliminados || 0}</td></tr>
                <tr><td style="padding: 4px 0;">Tablas optimizadas (VACUUM):</td><td style="text-align: right; font-weight: bold;">${acciones.tablas_optimizadas || 0}</td></tr>
                <tr><td style="padding: 4px 0;">Estadísticas de índices:</td><td style="text-align: right; color: #10b981; font-weight: bold;">Actualizadas</td></tr>
              </table>

              <h3 style="color: #0f172a; border-bottom: 2px solid #f1f5f9; padding-bottom: 8px; margin-top: 25px;">2. Auditoría de Integridad</h3>
              <table style="width: 100%; font-size: 13px;">
                <tr><td style="padding: 4px 0;">Recibos con deuda $0 (Revisar):</td><td style="text-align: right; color: ${hallazgos.recibos_vacios > 0 ? '#ef4444' : '#10b981'}; font-weight: bold;">${hallazgos.recibos_vacios || 0}</td></tr>
                <tr><td style="padding: 4px 0;">Registros sin propietario:</td><td style="text-align: right; font-weight: bold;">${hallazgos.sin_propietario || 0}</td></tr>
              </table>
              ...
              <div style="background: #f8fafc; padding: 25px; border-radius: 15px; margin-top: 30px; border: 1px solid #f1f5f9; text-align: center;">
                <div style="display: inline-block; width: 30%;">
                  <p style="font-size: 24px; font-weight: 800; margin: 0; color: #0f172a;">${totales.edificios || 0}</p>
                  <p style="font-size: 9px; text-transform: uppercase; color: #94a3b8;">Edificios</p>
                </div>
                <div style="display: inline-block; width: 30%;">
                  <p style="font-size: 24px; font-weight: 800; margin: 0; color: #0f172a;">${totales.recibos || 0}</p>
                  <p style="font-size: 9px; text-transform: uppercase; color: #94a3b8;">Recibos</p>
                </div>
                <div style="display: inline-block; width: 30%;">
                  <p style="font-size: 24px; font-weight: 800; margin: 0; color: #0f172a;">${totales.movimientos || 0}</p>
              ...
              <p style="font-size: 10px; color: #94a3b8; text-align: center; margin-top: 30px; border-top: 1px solid #f1f5f9; padding-top: 15px;">
                Mantenimiento Programado | Proyectado para cada 15 días | ${new Date().toLocaleString('es-VE')}
              </p>
            </div>
          </div>`
      });
    ...
    if (action === "backup") {
      ...
      const backupData: any = { version: "1.4", timestamp: new Date().toISOString(), tables: {} };
