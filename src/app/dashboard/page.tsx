...
export default function DashboardPage() {
  ...
  const addServicioConfig = async (tipo: string, identificador: string, alias: string, diaConsulta: number) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const err = await res.json();
        alert(`Error: ${err.error || "No se pudo agregar la configuración"}`);
      }
    } catch (error) {
  ...
  const removeServicioConfig = async (id: string) => {
    if (!confirm("¿Estás seguro de eliminar esta configuración de servicio?")) return;
    try {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="bg-green-600 text-white text-center py-2 font-black uppercase text-[10px] tracking-[0.2em] fixed top-0 left-0 right-0 z-[999]">
                        ...
                        <div className="flex items-center gap-3">
                          {building?.url_recibo_mes && (
                            <a
                              ...
                              className="px-3 py-1.5 bg-green-600 text-white rounded-lg text-xs font-bold hover:bg-red-700 transition-colors flex items-center gap-1.5"
                            >
...
// BUILD REPAIR: 2026-04-27 12:25 VET
// FORCE UPDATE: Mon Apr 27 04:37:48 PM UTC 2026
