...
export default function DashboardPage() {
  ...
  const loadIntelligenceData = async () => {
    ...
    try {
      const res = await fetch(`/api/analytics/cobranza?edificioId=${building.id}`);
      const data = await res.json();
      if (res.ok) setIntelligenceData(data);
    } catch (e) {
      console.error("[Intelligence] Error cargando datos:", e);
    } finally {
  ...
  useEffect(() => {
    if (activeTab === "inteligencia" && building?.id) {
  ...
  const loadProyeccion = async () => {
    ...
    try {
      const res = await fetch(`/api/proyeccion?edificioId=${building.id}`);
      const data = await res.json();
      if (res.ok) {
            ...
            {(editConfig.dashboard_config?.usd !== false || user?.id === "superuser-id") && (
              <div className="grid md:grid-cols-4 gap-6">
                <div className="bg-white p-6 rounded-xl shadow-sm cursor-pointer hover:bg-gray-50 border border-gray-100 group" onClick={() => setActiveTab("balance")} title="Saldo actual según el portal de la administradora.">
                  <div className="text-sm text-gray-500 mb-1">Saldo seg&uacute;n Administradora</div>
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm cursor-pointer hover:bg-gray-50 border border-gray-100 group" onClick={() => setActiveTab("ingresos")} title="Total de cobranza recibida en el mes actual.">
                  ...
                  {tasaBCV.dolar > 0 && <div className="text-sm text-gray-400">$ {formatUsd((ingresosSummary?.monto || 0) / tasaBCV.dolar)}</div>}
                </div>
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm cursor-pointer hover:bg-gray-50 border border-gray-100 group" onClick={() => setActiveTab("balance")} title="Fondo de reserva acumulado según último balance.">
                  <div className="text-sm text-gray-500 mb-1">Fondo de Reserva</div>
            ...
            {(editConfig.dashboard_config?.cg !== false || user?.id === "superuser-id") && (
              <div className="grid md:grid-cols-4 gap-6">
                <div className="bg-white p-6 rounded-xl shadow-sm cursor-pointer hover:bg-gray-50 border border-gray-100 group" onClick={() => setActiveTab("egresos")} title="Pagos realizados en el mes actual.">
                  ...
                  {tasaBCV.dolar > 0 && <div className="text-sm text-gray-400">$ {formatUsd((egresosSummary?.monto || 0) / tasaBCV.dolar)}</div>}
                </div>
...
<div className="overflow-hidden border border-gray-200 rounded-xl">
                  ...
                  <table className="w-full text-sm text-left">
                      ...
                      {itemsFondos.length > 0 && (
                        <>
                          <tr className="bg-blue-50">
                            <td colSpan={5} className="py-2 px-4 text-[10px] font-black text-blue-800 uppercase tracking-widest">Aportes a Fondos y Reservas</td>
                          </tr>
                          {itemsFondos.map((item, idx) => (
                      ...
                      {itemsNoComunes.length > 0 && (
                        <>
                          <tr className="bg-amber-50">
                            <td colSpan={5} className="py-2 px-4 text-[10px] font-black text-amber-800 uppercase tracking-widest">Gastos No Comunes (Privativos)</td>
                          </tr>
                          {itemsNoComunes.map((item, idx) => (
                    ...
                    <tfoot className="bg-indigo-950 text-white font-bold border-t-2 border-indigo-900">
                      <tr>
                        <td colSpan={2} className="py-4 px-4 text-right uppercase text-xs font-black">TOTAL RECIBO DEL MES:</td>
                ...
                <div className="bg-gray-50 border-2 border-dashed border-gray-200 rounded-xl py-10 text-center">
                  <p className="text-gray-500 font-medium">No hay detalles disponibles para este periodo.</p>
                  <p className="text-[10px] text-gray-400 mt-1 uppercase font-bold italic">Sincroniza los datos para visualizar el detalle.</p>
                </div>
              )}
            </div>
          </div>
            ...
            <div id="junta-form" className="grid md:grid-cols-5 gap-4 mb-8 bg-gray-50 p-4 rounded-lg border border-gray-200">
              ...
              <div>
                ...
                <select 
                  ...
                  className={`w-full px-3 py-2 text-sm border ${newMiembro.nivelAcceso === 'admin' ? 'border-indigo-400 bg-indigo-50' : newMiembro.nivelAcceso === 'board' ? 'border-blue-400 bg-blue-50' : 'border-gray-300 bg-white'} rounded focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none font-bold transition-all`}
                >
                  <option value="viewer">👁️ SOLO VISUALIZACIÓN (LECTURA)</option>
                  <option value="board">📝 MIEMBRO JUNTA (EDICIÓN PARCIAL)</option>
        ...
        {activeTab === "proyeccion" && (
          <div className="space-y-8 animate-in fade-in duration-700">
            {!planInfo?.permissions?.hasAdvancedKpis ? (
              <UpgradeCard
                title="Proyección de Ingresos Inteligente"
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                ...
                <div className={`space-y-4 ${!planInfo?.permissions?.hasCustomDashboard ? 'opacity-40 grayscale pointer-events-none' : ''}`}>
                  ...
                  <button 
                    ...
                    className="w-full mt-4 py-3 bg-indigo-50 text-indigo-700 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-100 transition-colors"
                  >
